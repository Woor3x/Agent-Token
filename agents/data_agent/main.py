...
from .handler import DataAgentHandler

setup_logging()

_CAP_PATH = Path(__file__).with_name("capability.yaml")
_config = AgentConfig.load("data_agent", _CAP_PATH)
_cap = load_capability(_CAP_PATH)
_handler = DataAgentHandler(
    feishu_base=_config.feishu_base,
    oauth=FeishuOAuth(base=_config.feishu_base),
)
