...
class AgentServer:
    ...
    def create_app(self) -> FastAPI:
        ...
        async def invoke(request: Request) -> JSONResponse:
            ...
            # Plumb optional X-Subject-Token (raw user JWT) through to the
            # handler — the orchestrator needs it as the ``subject_token`` for
            # subsequent IdP token-exchange calls when fanning out to peers.
            subject_token = request.headers.get("X-Subject-Token")
            if subject_token:
                ctx = body.setdefault("context", {})
                ctx["subject_token"] = subject_token
