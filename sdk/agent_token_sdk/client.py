...
class AgentClient:
    ...
    def __init__(
        ...
        *,
        ...
        # M1 IdP verifies client_assertion ``aud == idp_issuer + /token/exchange``.
        # Default to env-driven issuer base so the SDK works inside compose
        # without callers having to plumb both URLs.
        self._assertion_audience = (
            assertion_audience
            or os.environ.get("IDP_ASSERTION_AUDIENCE")
            or f"{os.environ.get('IDP_ISSUER', 'https://idp.local')}/token/exchange"
        )
    ...
    async def invoke(
        ...
        *,
        ...
        headers = {
            ...
            # Forward original user/subject token so the callee can fan out
            # with it as ``subject_token`` for its own token-exchange calls.
            "X-Subject-Token": on_behalf_of,
    ...
    async def _token_exchange(
        ...
        *,
        ...
        assertion = self._assertion.sign(
            ...
            exp_delta=self._assertion_exp_delta,
        )
